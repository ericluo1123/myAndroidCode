package com.teamsourcing.LoongyeeApp.controls;
...
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener {
     ...
     * Max theta
     */
    private static final float MAX_THETA = 5.0f;

    /**
     * Duration in milliseconds from the start of a scroll during which we're
     * unsure whether the user is scrolling or flinging.
     */
    private static final int SCROLL_TO_FLING_UNCERTAINTY_TIMEOUT = 500;
     ...
     * The axe angle
     */
    private float mTheta = (float)(5.0f*(Math.PI/180.0));
    ...
    void setSelectedPositionInt(int position) {
        ...
        if(showText != null) {
            if (position == 2) {
                showText.setText(R.string.light_name);
            } else if(position == 4){
                showText.setText(R.string.server_name);
            }
        }
    ...
    private void setUpChild(CarouselImageView child, int index, float angleOffset) {
        ...
        if(mInLayout)
        {
            h = (getMeasuredHeight() - getPaddingBottom() - getPaddingTop())/2;
